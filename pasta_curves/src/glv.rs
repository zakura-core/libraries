...
/// The outputs must be distinct, and every denominator must be nonzero.
fn batch_invert_and_add<F: Field>(
    ...
    let product_inverse = product.invert().unwrap();
...
pub(crate) fn try_multiexp<C: GlvParams>(
    ...
    Some(multiexp(&components, &bases, window_bits, num_threads))
}
